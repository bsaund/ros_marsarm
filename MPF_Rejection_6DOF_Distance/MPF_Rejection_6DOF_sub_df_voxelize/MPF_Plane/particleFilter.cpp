...
void importSTL(vector<vec4x3> &mesh);
/*
...
void particleFilter::addObservation(double obs[3], vector<vec4x3> &mesh, distanceTransform *dist_transform, int idx_obs)
{
		...
		double R, double Xstd_ob, double Xstd_tran)
{
	...
	while (i < n_particles)
	{
		...
		D = (*dist_transform->dist_transform)[int(floor((cur_inv_M[0] - dist_transform->world_range[0][0]) / dist_transform->voxel_size))]
										     ...
										     [int(floor((cur_inv_M[2] - dist_transform->world_range[2][0]) / dist_transform->voxel_size))];
...
int main()
{
	vector<vec4x3> mesh(12);
	hashmap boundary_voxel;
	importSTL(mesh);
	//double point[3] = { 2.9999,2.0001,0 };
	//cout << checkInObject(mesh, point) << endl;
