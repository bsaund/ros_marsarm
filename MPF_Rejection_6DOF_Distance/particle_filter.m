...
X_true=[2.11,1.388,0.818, pi/6+pi/400, pi/12+pi/420, pi/18-pi/380];
box_para=[6,4,2];
range = [-3.5, 3.5; -2.5, 2.5; -1.5, 1.5];
b_Xprior=[Xprior;Xprior_std];
b_Xpre=b_Xprior;
X0 = create_particles(b_Xprior, N);
[dist_transform, object_map] = distance_transform(box_para, range, grid_res);
X_1=X0;
X_ini=X0;
N_Measure=90;
M_std=0.0001;
...
for k = 1:60
    ...
    [X, iffar] = update_particles(X_1, X0, Xstd_ob, Xstd_tran, R, box_para, M(:,k), k, dist_transform, range, grid_res);
    if k==1
        X0=X;
    elseif iffar == 1
        X0=X_1;
