function [X, iffar] = update_particles(X_1, X0, Xstd_ob, Xstd_tran, R, box_para, M, k, dist_transform, range, grid_res)
N=length(X0(1,:));
X = zeros(3,N);
count=0;
i=0;
...
while count < N
    ...
    %D2=dist_transform(ceil((M_inv(1)-range(1,1))/grid_res), ceil((M_inv(2)-range(2,1))/grid_res), ceil((M_inv(3)-range(3,1))/grid_res));
    i=i+1;
end
