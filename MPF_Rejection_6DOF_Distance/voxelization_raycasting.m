[v, f, n, c, stltitle] = stlread('binaryCube.STL');
    ...
    for ix = pstart(1):voxel_size:pend(1)
        for iy = pstart(2):voxel_size:pend(2)
            for iz = pstart(3):voxel_size:pend(3)
                ...
                if (dist >= sqrt(2)R - Xstd_ob && dist <= R + Xstd_ob)                   
                    p=[ix,iy,iz] - dist .*n(i,:);
                    w2 = p - v0;
                    d20 = dot(w2,w0);
                    d21 = dot(w2,w1);
...
%                                 plot3(ix,iy,iz,'.r');
% %                                 plot3(p(1),p(2),p(3),'.k');
% %                                 plot3(v0(1),v0(2),v0(3),'.k','MarkerSize',20);
% %                                 plot3(v1(1),v1(2),v1(3),'.k','MarkerSize',20);
% %                                 plot3(v2(1),v2(2),v2(3),'.k','MarkerSize',20);
% %                                 p(1)
% %                                 p(2)
% %                                 p(3)
% %                                 alpha
