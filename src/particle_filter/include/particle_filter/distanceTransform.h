...
void distanceTransform::distanceTransform_1D(double ***dist_transform_1D, int range[3], double ***cost_fun, int dir, int idx1, int idx2)
{
  ...
  if (dir == 0) {
    for (int q = 1; q < range[dir]; q++) {
      s = ((cost_fun[q][idx1][idx2] + double(q * q)) 
	   - (cost_fun[envelope[k]][idx1][idx2] 
	      + double(envelope[k] * envelope[k]))) / double(2 * q - 2 * envelope[k]);
      while (s <= bound[k])
	{
	  k = k - 1;
	  s = ((cost_fun[q][idx1][idx2] + double(q * q)) 
    ...
    for (int q = 1; q < range[dir]; q++) {
      s = ((cost_fun[idx1][q][idx2] + double(q * q)) 
	   - (cost_fun[idx1][envelope[k]][idx2] 
	      + double(envelope[k] * envelope[k]))) 
	/ double(2 * q - 2 * envelope[k]);
      while (s <= bound[k])
	{
	  k = k - 1;
	  s = ((cost_fun[idx1][q][idx2] + double(q * q)) 
	...
	k = k + 1;
      ...
      dist_transform_1D[idx1][q][idx2] = 
	double((q - envelope[k]) * (q - envelope[k]))
	+ cost_fun[idx1][envelope[k]][idx2];
    }
  }
  else {
    for (int q = 1; q < range[dir]; q++) {
      double t = (cost_fun[idx1][idx2][q] + double(q * q)) - 
	(cost_fun[idx1][idx2][envelope[k]]);
      s = ((cost_fun[idx1][idx2][q] + double(q * q)) - 
	   (cost_fun[idx1][idx2][envelope[k]] + 
	    double(envelope[k] * envelope[k]))) / double(2 * q - 2 * envelope[k]);
      while (s <= bound[k])
	{
...
void distanceTransform::build(double cube_para[3], double World_Range[3][2])
{
  ...
  double x, y, z;
  memcpy(world_range, World_Range, 6 * sizeof(double));
  std::cout << "world_range: " << world_range[0][0] << ", " << world_range[0][1] << ", " << world_range[1][0] << ", " << world_range[1][1] << ", " << world_range[2][0] << ", " << world_range[2][1] << std::endl;
  std::cout << "cube_para: " << cube_para[0] << ", " << cube_para[1] << ", " << cube_para[2] << std::endl;
  for (int i = 0; i < voxel_nums[0]; i++) {
    for (int j = 0; j < voxel_nums[1]; j++) {
	...
	if(i%10==0 && j%10==0 && k%10==0){
	  /* std::cout << fabs(x)<<", "<<fabs(y)<<", "<<fabs(z) << "|;| "; */
	  /* std::cout << obstacle_map[i][j][k] << ", "; */
	}
      }
    }
  }
  for (int i = 0; i < voxel_nums[0]; i++) {
    ...
    for (int k = 0; k < voxel_nums[2]; k++) {
      ...
      /* distanceTransform_1D(dist_transform, voxel_nums, obstacle_map, 1, i, k); */
    }
  }
